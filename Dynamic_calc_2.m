...
fa_square = 5*0.5^2; % constant, check article for proving stage

% za = sqrt(fa_square/f);
f_values = 1:1:90; % f values from 2 to 100 with step 2
za_all = sqrt(fa_square./f_values);

Tr_values = zeros(size(f_values)); % Preallocate for Tr values
...
for i = 1:length(f_values)
    ...
    [t,phi_dyn] = ode45(@(t,phi) myODE_dyn_1(t,phi,b,phi_0,g,f,za,kh,M), tspan_1, phi0);
    ...
    phi_ddot = 1./(2*b*cos(phi_dyn(:, 1))) .* ( 2*b*sin(phi_dyn(:, 1)).*phi_dyn(:, 2).^2 +...
        4*pi^2*f.^2*za*sin(2*pi.*f.*t) -g  + (9*kh*b)/(2*M)*tan(phi_dyn(:, 1)).*(cos(phi_dyn(:, 1)) - cos(phi_0)*ones(size(phi_dyn(:, 1)))  )       );
    ...
    idx = t >= 1 & t <= 2;
    max_value = max(z_ddot(idx) + 2*b*cos(phi_all(idx,1)).*phi_all(idx,3) - 2*b*sin(phi_all(idx,1)).*phi_all(idx,2));
    max_z_ddot = max(abs(z_ddot));

    % Transmissibility
    Tr = max_value / max_z_ddot;
    Tr_values(i) = 5*log10(Tr);
end

figure()
semilogx(f_values, Tr_values, 'b', LineWidth=1.5)
xlabel('Frequency [Hz]');
ylabel('Transmissibility [dB]');
title('Transmissibility vs. f')
grid on
